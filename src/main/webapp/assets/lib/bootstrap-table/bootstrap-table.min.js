...
function(a) {
    ...
    p.LOCALES["en-US"] = p.LOCALES.en = {
        ...
        formatShowingRows: function(a, b, d) {
            //return c("显示第 %s 到第 %s 条记录，总 共 %s 条记录", a, b, d)
        	return "";
        },
    ...
    p.prototype.initServer = function(b, c, d) {
        ...
        this.options.pagination && (i.pageSize = this.options.pageSize, i.currentPageNo = this.options.pageNumber
        /* i.offset=this.options.pageSize===this.options.formatAllRows()?0:this.options.pageSize*(this.options.pageNumber-1),i.limit=this.options.pageSize===this.options.formatAllRows()?this.options.totalRows:this.options.pageSize */
        )), a.isEmptyObject(this.filterColumnsPartial) || (i.filter = JSON.stringify(this.filterColumnsPartial, null)), g = h(this.options, this.options.queryParams, [i], g), a.extend(g, c || {}), g !== !1 && (b || this.$tableLoading.show(), e = a.extend({},
