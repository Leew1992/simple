...
public class MailManager {
    ...
    public static final String SMTP_HOST = "smtp.126.com";

    // 收件人邮箱（替换为自己知道的有效邮箱）
    public static final String RECIPIENT = "776379740@qq.com";
    
	/**
	 * 发送普通邮件
	 */
	public static void sendMail(MailSendRecordDTO mailSendRecordDTO) throws Exception {
        ...
        // 3. 创建一封邮件
        MimeMessage message = createMimeMessage(session, mailSendRecordDTO);
	 ...
	 * @throws Exception 
	 */
	public static void sendSSLMail(MailSendRecordDTO mailSendRecordDTO) throws Exception {
		// 1. 创建参数配置, 用于连接邮件服务器的参数配置
        Properties props = new Properties();                    // 参数配置
        props.setProperty("mail.transport.protocol", "smtp");   // 使用的协议（JavaMail规范要求）
        props.setProperty("mail.smtp.host", SMTP_HOST);   // 发件人的邮箱的 SMTP 服务器地址
	 ...
	 * @throws Exception 
	 */
	public static void sendVerifyCodeByMail(String content, String recipient) throws Exception {
     ...
     * 创建一封只包含文本的简单邮件
     */
    private static MimeMessage createMimeMessage(Session session, MailSendRecordDTO mailSendRecordDTO) throws Exception {
        // 1. 创建一封邮件
